...
function DetailModal({
  ...
  onClose: () => void;
  onPurchase?: (row: Record<string, AnyValue>) => void;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-white/10 shrink-0 rounded-b-2xl bg-black/20 flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white font-medium text-sm transition-colors border border-white/10"
          >
            বন্ধ করুন
          </button>
          
          {onPurchase && row.can_be_purchased && (
            <button
              type="button"
              disabled={isPurchasing}
              onClick={() => onPurchase(row)}
              className="flex-1 py-2 rounded-lg bg-gradient-to-r from-primary-600 to-indigo-600 hover:from-primary-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-sm shadow-lg shadow-primary-500/20 transition-all flex items-center justify-center gap-2"
            >
              {isPurchasing ? (
                <>
      ...
      <AnimatePresence>
        {selectedRow && (
          <DetailModal
            ...
            onPurchase={(row) => handlePurchase(row as unknown as TicketSchedule)}
            isPurchasing={purchasingId === selectedRow.id}
          />
        )}
      </AnimatePresence>
    </div>
